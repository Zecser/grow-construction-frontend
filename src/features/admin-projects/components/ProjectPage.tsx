    ...
    const projectCards = [
        {
            ...
            items: ["Residential Apartment", "Commercial Hub", "Industrial Facility"],
            color: "bg-grayLight",
            footer: true,
        },
        {
            ...
            <main className="flex-1  p-4 sm:p-8 md:ml-[-2rem]  sm:ml-[-2rem] xs:ml-[-2rem] lg:mr-[10%] flex flex-col">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-6 w-full max-w-5xl mx-auto lg:w-[700px] lg:h-[500px] lg:ml-10">
                    {projectCards.map((card) => (
                        ...
                        >
                            <div className={`font-semibold text-lg mb-3 ${card.color.includes("text-white") ? "text-white" : "text-primary"}`}>{card.title}</div>
                            {/* Project Items */}
                            {card.isAdd ? (
                                ...
                                <>
                                    <ul className="flex-1 mb-4 space-y-1">
                                        {card.items.map((item) => (
                                            <li key={item} className={`${card.color.includes("text-white") ? "text-white" : "text-[#222]"} text-base`}>{item}</li>
                                        ))}
                                    </ul>
                                    {card.footer && (
                                        <button
                                            className={`flex items-center md:pl-15 lg:pl-45 xl:pl-45 gap-1 text-sm font-medium mt-auto ${card.color.includes("text-white") ? "text-white/80 " : "text-primary"}`}
                                            onClick={() => console.log(card.title + " Show More clicked!")}
                                        >
                                            Show More
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 6l6 6-6 6" /></svg>
                                        </button>
                                    )}
                                </>
                            )}
